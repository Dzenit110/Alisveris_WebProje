        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table" style="background:#ddd;font-weight: 600;" >
                    ...
                    <tbody>
        @foreach($siparisler1 as $key=> $siparis)
        <tr>
            <td>{{ $key+1 }}</td>
           ...
           <td>{{ $siparis->miktar }} TL</td>
            ...
            <td> {{ $siparis->iptal_sebep }}</td>
            <td> 
@if($siparis->iptal_siparis == 0)
<span class="badge rounded-pill bg-warning">İptal Talebi Yok</span>

@elseif($siparis->iptal_siparis == 2)
<span class="badge rounded-pill bg-danger">Beklemede</span>

@elseif($siparis->iptal_siparis == 3)
...
@endif
     ...
     <a href="{{ url('user/fatura_download/'.$siparis->id) }}" class="btn-sm btn-danger"><i class="fa fa-download"></i> Invoice</a>
            </td>
        </tr>
        @endforeach
